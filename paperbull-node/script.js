...
async function handleLoginSubmit(e){
  ...
  try {
    ...
    if (!resp.ok) {
      if (errBox) { errBox.textContent = data.error || 'Login failed.'; errBox.style.display = 'block'; }
      else alert(data.error || 'Login failed.');
      return;
    }

    // Store user info and redirect to the main dashboard
    sessionStorage.setItem('pbUser', JSON.stringify(data.user));
    window.location.href = 'index.html';
  } catch (err) {
    const msg = 'Could not reach the server. Is the backend running?';
...
async function handleSignupSubmit(e){
  ...
  if(password.value !== confirm.value){ alert("Passwords don't match."); confirm.focus(); return; }
  if(password.value.length < 8){ alert('Password must be at least 8 characters.'); password.focus(); return; }
  if(!terms.checked){ alert('Please agree to the Terms of Service and Privacy Policy.'); return; }

  try {
    const resp = await fetch('http://localhost:8000/api/signup', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ display_name: fullname.value, email: email.value, password: password.value }),
    });
    const data = await resp.json();
    if (!resp.ok) {
      if (errBox) { errBox.textContent = data.error || 'Signup failed.'; errBox.style.display = 'block'; }
      else alert(data.error || 'Signup failed.');
      return;
    }
    alert('Account created for ' + fullname.value + '! Please log in.');
